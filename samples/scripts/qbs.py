...
def main():
    ...
    if action_name == '-h':
        ...
        if arg_len >= 3:
            ...
            read(dir_path, excel_file, f_fail)
        else:
            print('read fail is\'t excel file')
...
def print_help():
    ...
    print('\tpython {0} {1}                转换目录中的视频文件'.format(
        SCRIPT_NAME, '-i'))
...
def get_source_info_ffmpeg(source_name):
    ...
    try:
        ...
        return_value = [filesize, duration,
                        codec_long_name, fps, bit_rate, width, height]
...
def read(dir_path, excel_file, f_fail):
    print('当前路径为          ---', dir_path, '开始分析 excel 文件')
    # 获取所有文件
    # file_list = get_all_file(dir_path)
    # print("文件读取完毕        --- 3秒后开始分析视频 ---")
    # time.sleep(3)
    # 解析文件
    dict_all = get_dict_info(excel_file, f_fail)
    # save_xlwt(dict_all)
    print("文件分析完毕")


def get_dict_info(excel_file, f_fail):
    dict_all = {}
    ...
    try:
        # info_list1 = get_dict_data(get_media_info(file))
        # 获取文件详情
        info_list = read_excel(excel_file)
        # ['13726665', '23/1', 2507776.0, 'mov,mp4,m4a,3gp,3g2,mj2', 720, 400]
        print('info_list', info_list)
        dict_all[filename] = info_list
        print('文件信息提取成功    ---')
